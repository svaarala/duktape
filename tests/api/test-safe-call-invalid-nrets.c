...
static duk_ret_t test_nrets_too_large(duk_context *ctx) {
	duk_int_t rc;
	 ...
	 */
	rc = duk_safe_call(ctx, dummy, 2 /*nargs*/, 1024 /*nrets, too large*/);
...
static duk_ret_t test_nrets_too_large_fixed(duk_context *ctx) {
	duk_int_t rc;
...
static duk_ret_t test_nrets_negative(duk_context *ctx) {
	duk_int_t rc;
	...
	rc = duk_safe_call(ctx, dummy, 2 /*nargs*/, -1 /*nrets, negative*/);
	printf("duk_safe_call rc: %ld\n", (long) rc);
