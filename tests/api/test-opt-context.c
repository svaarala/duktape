...
static duk_ret_t test_1(duk_context *ctx, void *udata) {
	...
	new_ctx = duk_opt_context(ctx, -1, NULL);
...
static duk_ret_t test_2(duk_context *ctx, void *udata) {
	...
	new_ctx = duk_opt_context(ctx, -1, (duk_context *) &dummy);
	printf("new_ctx is dummy: %d\n", (new_ctx == (duk_context *) &dummy ? 1 : 0));

	/* invalid index */
	new_ctx = duk_opt_context(ctx, 123, (duk_context *) &dummy);
	printf("new_ctx is dummy: %d\n", (new_ctx == (duk_context *) &dummy ? 1 : 0));

	/* invalid index */
	new_ctx = duk_opt_context(ctx, DUK_INVALID_INDEX, (duk_context *) &dummy);
	printf("new_ctx is dummy: %d\n", (new_ctx == (duk_context *) &dummy ? 1 : 0));
	return 0;
}
