...
const { Snippet, scanHelperSnippets } = require('./snippet.js');
const { FileBuilder } = require('./file_builder.js');
const { scanUseDefs } = require('./use_defs.js');
const { cIntEncode, cStrEncode } = require('../util/cquote.js');
...
function generateDukConfigHeader(opts) {
    ...
    var platforms = parseYaml(readFileUtf8(pathJoin(configDirectory, 'platforms.yaml')));
    var architectures = parseYaml(readFileUtf8(pathJoin(configDirectory, 'architectures.yaml')));
    var compilers = parseYaml(readFileUtf8(pathJoin(configDirectory, 'compilers.yaml')));
    ...
    ret.line('/*');
    ret.line(' *  duk_config.h configuration header generated by duktool.js');
    ret.line(' *');
