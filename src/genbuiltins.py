...
#
#  Various helper keys are also added, including (see more below):
#
#    _strings_plain: list of plain strings
#    _stridx_to_string: map from stridx to string object
#    _stridx_to_plain: map from stridx to plain string
#    _plain_to_stridx: map a plain string to stridx
#    _define_to_stridx: map a string C define to stridx
#    _stridx_to_define: map a stridx to a C define
#
...
def metadata_add_ram_filtered_object_list(meta):
	...
	for o in meta['objects']:
		...
		if o.has_key('native') and not o.has_key('bidx'):
			# Handled specially, see property init data code
			pass
		else:
			# Top level object
			keep = True
