...
DUK_EXTERNAL void duk_suspend(duk_context *ctx) {
	...
	snapshot = (duk_thread_state *) duk_push_fixed_buffer(ctx, sizeof(duk_thread_state));
...
DUK_EXTERNAL void duk_resume(duk_context *ctx) {
	duk_hthread *thr = (duk_hthread *) ctx;
	duk_heap *heap;
	const duk_thread_state *snapshot;
	duk_size_t snapshot_size;
	...
	snapshot = (const duk_thread_state *) duk_require_buffer(ctx, -1, &snapshot_size);
	DUK_ASSERT(snapshot_size == sizeof(duk_thread_state));
