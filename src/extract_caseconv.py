...
def main():
	...
	genc.emitArray(uc_bytes, opts.table_name_uc, bytesize=len(uc_bytes), typename='duk_uint8_t', intvalues=True)
	genc.emitArray(lc_bytes, opts.table_name_lc, bytesize=len(lc_bytes), typename='duk_uint8_t', intvalues=True)
	...
	genc = dukutil.GenerateC()
	genc.emitHeader('extract_caseconv.py')
	genc.emitLine('extern duk_uint8_t %s[%d];' % (opts.table_name_uc, len(uc_bytes)))
	genc.emitLine('extern duk_uint8_t %s[%d];' % (opts.table_name_lc, len(lc_bytes)))
