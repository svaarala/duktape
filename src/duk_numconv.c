 ...
 *  correct and uses a minimum number of digits.  Much faster
 *  algorithms exist (e.g. one could use Grisu3 + Dragon4 hybrid
 *  with much better average performance) but that would increase
 *  code footprint considerably.
 */
 ...
 *  numbers with less than 32 * MAX_PARTS bits.  Operations have been tailored
 *  for number conversion needs.
 */

#define  MAX_PARTS  35  /* 35x32 = 1120 bits */
...
typedef struct {
	int n;
	uint32_t v[MAX_PARTS];  /* low to high */
} duk_bigint;
...
#define  NUMCONV_PUTCHAR(nc_ctx,x)  do { \
		DUK_ASSERT((nc_ctx)->out_p < (nc_ctx)->out_buf); \
		*((nc_ctx)->out_p++) = (x); \
	} while(0)
...
static void convert_double(duk_numconv_stringify_ctx *nc_ctx) {
...
static void prepare(duk_numconv_stringify_ctx *nc_ctx) {
	...
	if (nc_ctx->e >= 0) {
		if (0/*FIXME*/) {
			 ...
			 * r <- (* f be1 2) == 2 * f * b^(e+1)     FIXME: if b==2 -> f * b^(e+2)
			 * s <- (* b 2)   FIXME: if b==2 -> 4
			 * m+ <- be1 == b^(e+1)
			 * m- <- be == b^e
			 * k <- 0
			 * B <- B
			 ...
			 * high_ok <- round
			 */
			DUK_DPRINT("e >= 0 AND (= f (expt b (- p 1)))");
			 ...
			 *
			 * be <- (expt b e) == b^e
			 * r <- (* f be 2) == 2 * f * b^e     FIXME: if b==2 -> f * b^(e+1)
			 * s <- 2
			 * m+ <- be == b^e
			 * m- <- be == b^e
			 * k <- 0
			 ...
			 * high_ok <- round
			 */
			DUK_DPRINT("e >= 0 AND (not (= f (expt b (- p 1))))");
...
static void scale(duk_numconv_stringify_ctx *nc_ctx) {
	int k = 0;

	/* FIXME: this loop can only go one way, i.e. increase k until it's
	 * correct, or decrease k until it's correct.
	 */
...
static void output(duk_numconv_stringify_ctx *nc_ctx, int d, int n) {
	int i;
	int pos;
...
void finish(duk_numconv_stringify_ctx *nc_ctx, int n) {
	int pos;
...
void generate(duk_numconv_stringify_ctx *nc_ctx) {
	int tc1, tc2;
	int d;
	int count = 0;
...
void duk_numconv_stringify(duk_context *ctx, double x, int radix, int digits) {
	...
	convert_double(nc_ctx);   /* -> sets 'f' and 'e' */
	BI_PRINT("f", &nc_ctx->f);
	...
	prepare(nc_ctx);  /* setup many variables in nc_ctx */
